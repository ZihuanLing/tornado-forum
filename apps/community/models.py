...
class CommunityGroupUser(BaseModel):
    user = ForeignKeyField(User, verbose_name='创建者')
    community = ForeignKeyField(CommunityGroup, verbose_name='社区')
    status = CharField(choices=HANDLE_STATUS, max_length=10, null=True, verbose_name='审核状态')
    handle_msg = CharField(max_length=200, null=True, verbose_name='处理内容')
    apply_reason = CharField(max_length=200, verbose_name='申请理由')
    handle_time = DateTimeField(default=datetime.now(), verbose_name='创建时间')
...
class PostComment(BaseModel):
    ...
    parent_comment = ForeignKeyField('self', verbose_name='原评论', null=True)
    ...
    def extend(cls):
        ...
        return cls.select(cls, Post, author.id, author.nick_name, author.head_url,
                          reply_user.id, reply_user.nick_name, reply_user.head_url)\
