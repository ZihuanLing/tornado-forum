...
class CommunityGroupForm(Form):
    ...
    desc = TextAreaField('简介', validators=[DataRequired('请输入简介')])
    notice = TextAreaField('公告', validators=[DataRequired('请输入公告')])
...
class HandleApplyForm(Form):
    status = StringField('处理结果', validators=[AnyOf(values=['agree', 'refuse'], message='只能为agree或refuse')])
